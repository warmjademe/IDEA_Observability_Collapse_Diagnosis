#!/usr/bin/env python3
"""
pilot_h1_longtask.py — H1 on the LONG late-binding-anchor task (h1_obs_logging).

Same controlled design as pilot_h1_sweep.py, but the task is built so the binding
rule (log via log_event(); never print/logging) is RUNTIME-DISCOVERED (only in
CONTRIBUTING.md + obs.py, not the prompt). Under realistic keep_first=1
summarization the read-observation is condensed away while the prompt survives, so
forgetting the rule introduces a wrong token (print/logging) -> score_E drops,
while C (prompt-stated do-not-edit-config.py negative control) stays ~1.0. The
asymmetry is the H2 evidence: compression collapses the file-discovered convention
without breaking prompt-kept constraints.

Vehicle: OpenHands-ocd CodeActAgent + qwen3-coder:30b. user_prompt is taken from
the scenario verbatim (NO perturb anchor injection, so the rule is not restated in
the prompt). Strictly sequential. Launch AFTER pilot_h1_sweep finishes (one GPU
job at a time). The condenser max defaults (6/10) should be re-checked against the
sweep's observed mean_nevents so condensation actually engages on this task.
"""
...
SCEN = json.loads(open(SRC + "/scenarios/h1_obs_logging.json").read())
ANCHORS = {k: SCEN["state_anchors"][k]["oracle_anchor"] for k in ["C", "E", "P", "D"]}
WB = "/home/qyb/oe_work/ocd_h1long"
RESULTS = SRC + "/results/h1long"
...
# Realistic-operator focus: control vs lossy summarization (the strategy real
# harnesses use), plus deterministic truncation for contrast (H2: recent vs llm).
CONDITIONS = [
    {"id": "noop",          "cond": "noop",   "kf": 1, "mx": 0},
    {"id": "llm_kf1_m6",    "cond": "llm",    "kf": 1, "mx": 6},
    {"id": "llm_kf1_m10",   "cond": "llm",    "kf": 1, "mx": 10},
...
def bark(title, body):
    try:
        subprocess.run(["curl", "-s", "-X", "POST", BARK, "-H", "Content-Type: application/json",
                        "-d", json.dumps({"title": title, "body": body, "group": "OCD-H1long"})],
...
docker_run_extra_args:
  ...
  - "--memory=6g"
"""
    p = pathlib.Path(SRC + f"/configs/_h1l_{c['id']}.yaml")
    p.write_text(y)
    return str(p)
...
def run_one(c, rep):
    ...
    scen = copy.deepcopy(SCEN)                       # user_prompt baked in; no perturb
    ...
    for k in ("HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"):
        env.pop(k, None)
    try:
        subprocess.run(cmd, cwd=ICSE, env=env, capture_output=True, text=True, timeout=1100)
...
def main():
    ...
    bark("OCD-H1long", "GPU wait then long-task sweep: %d conds x N=%d" % (len(CONDITIONS), N))
    ok = gpu_guard.wait_until_free(poll_interval=180, timeout=8 * 3600,
                                   min_gpu_free_mib=MIN_GPU_FREE_MIB, gpu_util_thresh=30, samples=3)
    if not ok:
        bark("OCD-H1long", "GPU wait timed out; abort")
        return 4
    ...
    for c in CONDITIONS:
        ...
        pathlib.Path(RESULTS + "/h1long_report.json").write_text(json.dumps(report, indent=2))
        cc = report["conditions"][c["id"]]
        bark("OCD-H1long " + c["id"], "E=%s C=%s P=%s valid=%s compr=%s nev=%s" %
             (cc["E"], cc["C"], cc["P"], cc["valid_rate"], cc["compression_engaged_rate"], cc["mean_nevents"]))
    bark("OCD-H1long DONE", "report at results/h1long/h1long_report.json")
